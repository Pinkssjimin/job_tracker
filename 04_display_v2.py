...
class Display:
    def __init__(self, partner, charges, names):
        ...
        # set up GUI frame
        self.show_jobs_frame = Frame(self.display_box, width=500, bg=background_colour)
        ...
        if len(charges) <= 0:
            ...
            job_info = "Job number: {}\nCustomer name: {}\nCharge: {}".format(job_num, names[job_num], charges[job_num])
        ...
        # job number
        self.job_display_label = Label(self.display_frame, text=job_info,
                                       font=label_font, justify=LEFT)
        self.job_display_label.grid(row=0, pady=10, padx=10)

        # export / add job buttons frame(row2)
        self.export_add_job_frame = Frame(self.show_jobs_frame)
        self.export_add_job_frame.grid(row=2, pady=10)
