# 13COS Jimin Ahn
# AS 91906
# 28/07/2022 - next and previous button
...
class Display:
    def __init__(self, partner, charges, names):
        ...
        self.job_displayed = 0 # first job entered
        ...
        # job number frame
        job_num_label = Label(self.display_frame, text="Job number:")
        job_num_label.grid(row=0, column=0, sticky="E", pady=10, padx=70)

        self.job_num_value = Label(self.display_frame)
        self.job_num_value.grid(row=0, column=1, pady=(0,10), sticky="W")

        # display label for name
        name_label = Label(self.display_frame, text="Name:")
        name_label.grid(row=1, column=0, pady=(0, 10), sticky="E")

        self.name_value_label = Label(self.display_frame)
        self.name_value_label.grid(row=1, column=1, pady=(0,10), sticky="W")

        # display label for charge
        charge_label = Label(self.display_frame, text="Charge:")
        charge_label.grid(row=2, column=0, sticky="E")

        self.charge_value_label = Label(self.display_frame)
        self.charge_value_label.grid(row=2, column=1, sticky="W")

        # next and previous button frame
        self.next_prev_frame = Frame(self.show_jobs_frame)
        self.next_prev_frame.grid(row=2, pady=10)

        #label to display number of jobs
        self.num_tracker = Label(self.next_prev_frame)
        self.num_tracker.grid(row=1, columnspan=2, pady=10)

        # Previous button
        self.prev_btn = Button(self.next_prev_frame, text="<Previous",
                               command=self.prev_job(charges, names))
        self.prev_btn.grid(row=0, column=0, padx=10)

        # next button
        self.next_btn = Button(self.next_prev_frame, text="Next>",
                               command=self.next_job(charges, names))
        self.next_btn.grid(row=0, column=1, padx=10)

        # export / add job buttons frame(row3)
        self.export_add_job_frame = Frame(self.show_jobs_frame)
        self.export_add_job_frame.grid(row=4, pady=10)
    ...
    def prev_job(self, charges, names):
        self.job_displayed -= 1
        self.show_job(charges, names)

    def next_job(self, charges, names):
        self.job_displayed += 1
        self.show_job(charges, names)

    def show_job(self, charges, names):
        # when there's no job entered
        if len(charges) == 0:

            self.num_tracker.configure(text='''Job 0/0
            No jobs entered''')

        # else display the jobs in list
        else:
            self.display_frame.grid()
            # disable next button if it's the last job in list
            if self.job_displayed == len(charges) - 1:
                self.next_btn.configure(state=DISABLED)

            # disable previous button if it's first job in list
            if self.job_displayed == 0:
                self.prev_btn.configure(state=DISABLED)

            # displaying the job information's
            self.job_num_value.configure(text=len(charges) + 1)
            self.num_tracker.configure(text="{}/{}".format(self.job_displayed + 1, len(charges)))
            self.name_value_label.configure(text=names[self.job_displayed])
            self.charge_value_label.configure(text="${:.2f}".format(charges[self.job_displayed]))
