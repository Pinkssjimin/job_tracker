...
class Job_tracker:
    def __init__(self, partner):
        ...
        partner.add_job_btn.config(state=DISABLED)

        background_colour = "light pink"
        entry_font = "Arial 11"

        # sets up child window
        self.jobs_box = Toplevel()

        # if users press cross at top, closes help and 'releases' help button

        self.jobs_box.protocol('WM_DELETE_WINDOW', partial(self.close_jobs, partner))

        self.charges = []
        self.names = []

        # job tracker frame
        self.job_frame = Frame(self.jobs_box, bg=background_colour)
        self.job_frame.grid()
        ...
        self.show_jobs_btn = Button(self.save_btn_frame, text="Show all jobs",
                                    command=partial(self.close_jobs, partner))
        self.show_jobs_btn.grid(row=1, columnspan=2)
...
class Display:
    def __init__(self):
        ...
        # set up GUI frame
        self.show_jobs_frame = Frame(bg=background_colour, pady=10)
        ...
        # display frame
        self.display_frame = Frame(self.show_jobs_frame)
        self.display_frame.grid(row=1)

        # job number
        self.job_num_label = Label(self.display_frame, text="Job number: ", font=label_font,
                                   highlightbackground="black", highlightthickness=1)
        self.job_num_label.grid(row=0, column=1, pady=10, padx=10)

        self.add_job_btn = Button(self.show_jobs_frame, text="Add new job",
                                  command=lambda: self.add_job())
        self.add_job_btn.grid(row=3, columnspan=2, padx=10, pady=10)
